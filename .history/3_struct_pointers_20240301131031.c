...
int main()
{
    ...
    for( ; ; )
    {
        printf("\nDo you want to enter details of a%s cat (Y or N)? ",first == NULL?"" : "");
        scanf(" %c", &test );
        if(tolower(test) == 'n')
            break;
        ...
        if (first==NULL)
        {
            ...
            current->previous = NULL;       //εφόσον είναι ο πρώτος κόμβος, δεν υπάρχει previous
        }    
        else
        {
            last->next = current;           //επόμενος κόμβος
            current->previous = last;       //προηγούμενος κόμβος
        }
