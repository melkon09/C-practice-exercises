...
int LL_delete_after(LIST_PTR prev, elem *x)
{
    ...
    if (prev->next=NULL)    /* Elegxos an einai o teleutaios komvos */
        return FALSE;
...
/* LL_print(): Typonei to periexomeno mias syndedemenis listas */
...
/* LL_destroy(): Apodesmeuei to xwro pou exei desmeusei i lista */
void LL_destroy(LIST_PTR )