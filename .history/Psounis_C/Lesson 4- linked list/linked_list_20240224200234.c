...
int LL_delete_start(LIST_PTR *head, elem *x)
{
    ...
    current=*head;          /* Gia na kseroume ti tha diagrapsoume */
    *x=current->data;

    (*head)=(*head)->next;  /* O head pleon deixnei ton deutero komvo (tha ginei prwtos pleon) */
    free(current);          /* Apodesmeusi xorou */
    return TRUE;    
}